...
public class AFPC_PlayerMovement_Editor : Editor {
	...
	private AnimBool showExtraFields, showExtraFields1, showExtraFields2, showExtraFields3, showExtraFields4, showExtraFields5, showExtraFields6, showExtraFields7, showExtraFields8, showExtraFields9, showExtraFields10, showExtraFields11, showExtraFields12, showExtraFields13, showExtraFields14;
	...
	private void OnEnable()
	{
		...
		showExtraFields13 = new AnimBool (true);
		showExtraFields13.valueChanged.AddListener (Repaint);
		showExtraFields14 = new AnimBool (true);
		showExtraFields14.valueChanged.AddListener (Repaint);
	}
		...
		} else if (afpcPlayer.playerType == AFPC_PlayerMovement.PlayerType.rigidBodyPlayer) 
		{
			...
			if (EditorGUILayout.BeginFadeGroup (showExtraFields3.faded)) 
			{
				...
				if (afpcPlayer.CANMOVE) {
					...
					LayerMask temp = EditorGUILayout.MaskField ("Layer Mask: ", InternalEditorUtility.LayerMaskToConcatenatedLayersMask (afpcPlayer.layerMask), InternalEditorUtility.layers, "ExposablePopupMenu");
			...
			if (EditorGUILayout.BeginFadeGroup (showExtraFields10.faded)) {
				...
				if (afpcPlayer.CANCROUCH) {
					...
					afpcPlayer.floatOffset = (float)EditorGUILayout.FloatField ("Float Offset For Prone/Crouch\nCollision Detection CapsuleCheck: ", afpcPlayer.floatOffset);
			...
			showExtraFields14.target = EditorGUILayout.Foldout (showExtraFields14.target, "Prone", showExtraFields14.target);
			if (EditorGUILayout.BeginFadeGroup (showExtraFields14.faded)) {
				EditorGUI.indentLevel++; 
				EditorGUILayout.BeginVertical ("WindowBackground");
				afpcPlayer.CANPRONE = EditorGUILayout.Toggle ("Can Prone: ", afpcPlayer.CANPRONE);
				if (afpcPlayer.CANPRONE)
				{
					afpcPlayer.proneAccelerationForward = (float)EditorGUILayout.FloatField ("Prone Acceleration Forward: ", afpcPlayer.proneAccelerationForward);
					afpcPlayer.proneAccelerationBackward = (float)EditorGUILayout.FloatField ("Prone Acceleration Backward: ", afpcPlayer.proneAccelerationBackward);
					afpcPlayer.proneStrafeAcceleration = (float)EditorGUILayout.FloatField ("Prone Strafe Acceleration: ", afpcPlayer.proneStrafeAcceleration);
					afpcPlayer.currentProneStandingRatio = (float)EditorGUILayout.IntSlider ("Prone To Standing Ratio: ", (int)afpcPlayer.currentProneStandingRatio, 0, 1);
					afpcPlayer.floatOffset = (float)EditorGUILayout.FloatField ("Float Offset For Prone/Crouch\nCollision Detection CapsuleCheck: ", afpcPlayer.floatOffset);
					afpcPlayer.proneKey = (KeyCode)EditorGUILayout.EnumPopup ("Prone Key: ", afpcPlayer.proneKey, "ExposablePopupMenu");
					afpcPlayer.originalLocalScaleY = (float)EditorGUILayout.FloatField ("Original Local Y Scale Of Player: ", afpcPlayer.originalLocalScaleY);
					afpcPlayer.transitionTimeOfProne = (float)EditorGUILayout.FloatField ("Transition Time For Prone/UnProne: ", afpcPlayer.transitionTimeOfProne);
					afpcPlayer.proneHeightRatio = (float)EditorGUILayout.Slider ("Prone Height Ratio: ", afpcPlayer.proneHeightRatio, 0f, 1f);
					EditorGUILayout.HelpBox ("Prone To Standing Ratio Determines whether the player is standing or Prone, set this value to 1 if you want player to be standing when the game starts and 0 if you want player to be Prone when the game starts!.\nFloat Offest Is how much above the player, the Collision Detection sphere will Sweep for Collision Detection.\nOriginal Local Scale Y is the original length or height of the player transform along local y axis.\nTransition Time Of Prone/UnProne is the time taken in seconds to go prone or unprone.", MessageType.Info, true);
				}
				EditorGUILayout.EndVertical ();
				EditorGUI.indentLevel--;
			}
			...
			showExtraFields13.target = EditorGUILayout.Foldout (showExtraFields13.target, "Tilting", showExtraFields13.target);
			if (EditorGUILayout.BeginFadeGroup (showExtraFields13.faded)) {
				EditorGUI.indentLevel++; 
				EditorGUILayout.BeginVertical ("WindowBackground");
				afpcPlayer.allowPlayerTilting = EditorGUILayout.Toggle ("Allow Tilting", afpcPlayer.allowPlayerTilting);
				if (afpcPlayer.allowPlayerTilting) {
					afpcPlayer.timeToTilt = (float)EditorGUILayout.FloatField ("Time To Tilt: ", afpcPlayer.timeToTilt);
					afpcPlayer.leftTiltKey = (KeyCode)EditorGUILayout.EnumPopup ("Tilt Left Key: ", afpcPlayer.leftTiltKey, "ExposablePopupMenu");
					afpcPlayer.rightTiltKey = (KeyCode)EditorGUILayout.EnumPopup ("Tilt Right Key: ", afpcPlayer.rightTiltKey, "ExposablePopupMenu");
					afpcPlayer.angleToTilt = (float)EditorGUILayout.Slider ("Angle To Tilt: ", afpcPlayer.angleToTilt, 0f, 90f);
				}
				EditorGUILayout.EndVertical ();
				EditorGUI.indentLevel--;
			}
			...
			if (EditorGUILayout.BeginFadeGroup (showExtraFields12.faded)) 
			{
				...
				if(afpcPlayer.playerType == AFPC_PlayerMovement.PlayerType.rigidBodyPlayer)
				{
					...
					afpcPlayer.proneButton = (AFPC_VirtualButton)EditorGUILayout.ObjectField("Prone Virtual Button: ", afpcPlayer.proneButton, typeof(AFPC_VirtualButton), true);
				...
				if (afpcPlayer.allowPlayerTilting) 
				{
					afpcPlayer.leftTiltVButton = (AFPC_VirtualButton)EditorGUILayout.ObjectField("Tilt Left VButton: ", afpcPlayer.leftTiltVButton, typeof(AFPC_VirtualButton), true);
					afpcPlayer.rightTiltVButton = (AFPC_VirtualButton)EditorGUILayout.ObjectField("Tilt Right VButton: ", afpcPlayer.rightTiltVButton, typeof(AFPC_VirtualButton), true);
				}
				EditorGUILayout.EndVertical ();
				EditorGUI.indentLevel--;
			}
