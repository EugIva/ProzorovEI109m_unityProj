		...
		if (CheckForDoorOrKey ()) {
			...
			if (_door != null) 
			{
				...
				if (doorPressed && !_door.requireKey) {
					...
					doorPressed = false;
				}else if(!_door.hasDoorKey && _door.requireKey && doorPressed && _doorKey == null)
				{
					//Just to Show notification in the demo scene
					if(_door.gameObject.GetComponent<AFPC_Door_KeyNTFC> ())
						_door.gameObject.GetComponent<AFPC_Door_KeyNTFC> ().StartCoroutine(_door.gameObject.GetComponent<AFPC_Door_KeyNTFC> ().ShowNotification ());
					doorPressed = false;
				}
			}
