...
public class AFPC_PlayerMovement : MonoBehaviour {
	...
	{
		Standing_0, Crouch_1, Swim_2, Climb_3, Prone_4	// These are the player states that player can have
	}
	...
	public AFPC_VirtualButton jumpButton, crouchButton, proneButton, runButton, goDownButton, spectatorForwardButton, spectatorDownButton;// Self Explanatory


	public bool CANMOVE = true, CANJUMP = true, CANCLIMB = true, CANSWIM = true, CANCROUCH = true, CANPRONE = true; // If they are true, then player can perform these specific actions
	...
	*/
	...
	private GameObject pivot;	/* The Gameobject according to which the player localscale should be 
								 * changed when crouching or Going Prone in order to remain grounded
	...
	public float floatOffset = 0.05f; /* Float Offest Is how much above the player, the
									   * crouch & prone Collision Detection capsule will Sweep for Collision Detection
									  */ 
	...
	[Header("Prone")]
	public float proneAccelerationForward = 10f, proneAccelerationBackward = 6f, proneStrafeAcceleration = 8f;
	public float currentProneStandingRatio = 0f; // If it is 1, then player is standing, if it is 0 then player is prone
	public KeyCode proneKey = KeyCode.Z; 
	[Range(0, 1)]
	public float proneHeightRatio = 0.1f; //The ratio of prone height to original height, if it is 0, player will be on the bottom most and if it is 0.5, the prone height will be in the middle 	 
	[Tooltip("In Seconds")]
	public float transitionTimeOfProne = 0.4f;
	...
	[HideInInspector]
	public bool playerOnWaterSurface = false; // determines whether the player is on water surface or not, useful in oxygen counter
	[HideInInspector]
	public bool isPlayerUnderWater = false; // Determines whether the player is underwater surface or not

	public Camera fpsCamera;
	public CapsuleCollider playerCapsule;


	private Vector3 crouchV, proneV; // used in smoothdamp
	...
	private float crouchLocalScaleY, proneLocalScaleY; // Local Scale of player in y axis when crouching, Going Prone
	private bool isJumping = false; // If true, then player is jumping
	private bool isWalking = false; // If true, then player is walking
	...
	public bool IsProne{ set{ isProne = value;} get{ return isProne;}}
	...
	private float zRotationV, zRotationV1; // used in smooth damp
	...
	private float playerCapsuleInitHeight, playerCapsuleInitRadius, radiusV;
	private bool decreaseRadiusProne = true, decreaseRadiusCrouch = false;
	...
	private float inputY3; //Vertical Input For Spectator Mode
	#if UNITY_STANDALONE || UNITY_WEBGL
	private bool swimKeyHold;
	#endif
	[HideInInspector]
	public bool jumpPressed, climbKeyHold, runKeyHold, crouchButtonDown, proneButtonDown, spectatorKeyHold;
	[HideInInspector]
	...
	void Start ()
	{
		...
		{
			playerCapsuleInitRadius = GetComponent<CapsuleCollider> ().radius;
			playerCapsuleInitHeight = GetComponent<CapsuleCollider> ().height;
		}
		...
		if (playerType == PlayerType.rigidBodyPlayer) {
			...
			pivot = new GameObject ("AFPC_PlayerPivot");	
	...
	//		pivot.transform.SetSiblingIndex (transform.GetSiblingIndex () + 1); // sets the index in hierarchy to be one before the afpcPlayer
			...
			if (currentProneStandingRatio == 0)
				isProne = true; // If the Player Set the prone standing ratio to 0, then player should be going prone
			...
			proneLocalScaleY = transform.localScale.y * proneHeightRatio;	// Set the prone local scale in Y axis to proneHeight Ratio Times Original Local scale in Y axis
			#if UNITY_STANDALONE || UNITY_WEBGL
	...
	public bool AboveCollisionDetection()
	{
		...
		endOfCapsule.y = transform.position.y + (playerCapsuleInitHeight * originalLocalScaleY / 2) + 0.25f + floatOffset;
		bool p = false;
		if (Physics.Raycast (transform.position, transform.up, endOfCapsule.y - transform.position.y, layerMask, QueryTriggerInteraction.Ignore))
			p = true;
		return p;

		//Returns TRUE if there are any colliders overlapping the Ray in world coordinates.
	}
	...
	private bool haltProneScale = false, haltCrouchScale = false;
	#region Crouching
	...
	void CrouchSystem()
	{
		...
		if(transform.localScale.y >= originalLocalScaleY - 0.01f)
		{
			if (decreaseRadiusCrouch && !isCrouching)
				playerCapsule.radius = Mathf.SmoothDamp(playerCapsule.radius, playerCapsuleInitRadius, ref radiusV, 0.15f);
		}
	...
	void DoCrouch()
	{
		isCrouching = true;
		isProne = false;
		currentProneStandingRatio = 1f;
		currentCrouchStandingRatio = 0f;
		if (decreaseRadiusCrouch)
			playerCapsule.radius = crouchLocalScaleY;
		// Radius of player capsule - 0.5f, if prone height or crouch height is less than player capsule's radius, then on UnCrouch/UnProne, Player Will bump!
		// To Stop This, We Will Set the radius of Player Capsule = Crouch Height/Prone Height!
	}
	...
	void ProneSystem()
	{
		...
		if (!haltProneScale)
		{
			float transformPosY = Mathf.Lerp (proneLocalScaleY, originalLocalScaleY, currentProneStandingRatio);	//Linearly Interpolate the transformPosY depending on the proneStanding Ratio
			Vector3 DesiredlocalScale = Vector3.SmoothDamp (transform.localScale, new Vector3 (transform.localScale.x, transformPosY, transform.localScale.z), ref proneV, transitionTimeOfProne);

			ScaleAround (transform, pivot.transform, DesiredlocalScale);
		}

		if (transform.localScale.y >= originalLocalScaleY - 0.01f) {
			if (decreaseRadiusProne && !isProne)
			{
				playerCapsule.radius = Mathf.SmoothDamp(playerCapsule.radius, playerCapsuleInitRadius, ref radiusV, 0.15f);
			}
		}
		if (isRunning && isProne /*If Run key is pressed while proned*/) 
		{
			DoUnProne ();
		}

		if (jumpPressed && isProne /*If Jump Key Is pressed during prone*/) 
		{
			DoUnProne ();
			jumpPressed = false;
		}
		#if UNITY_ANDROID || UNITY_IOS
		if(proneButton != null)
			proneButton.buttonType = AFPC_VirtualButton.ButtonType.LateTriggerButton;
		#endif
		if (proneButtonDown) 
		{
			if (!isProne) 
			{
				...
				{
					// If not already prone, then Go Prone 
					DoProne ();
				}
			} else 
			{
				//If prone, then UnProne
				DoUnProne ();
			}
			proneButtonDown = false;
		}
	}
	...
	void DoProne()
	{
		...
		isCrouching = false;
		currentCrouchStandingRatio = 1f;
		currentProneStandingRatio = 0f;
		if (decreaseRadiusProne)
			playerCapsule.radius = proneLocalScaleY;
		// Radius of player capsule - 0.5f, if prone height or crouch height is less than player capsule's radius, then on UnCrouch/UnProne, Player Will bump!
		// To Stop This, We Will Set the radius of Player Capsule = Crouch Height/Prone Height!
	}
	...
	#region Tilt System
	private bool prevLeftTiltPressed, prevRightTiltPressed;
	...
	{
		zRotation = Mathf.SmoothDamp (zRotation, 0f, ref zRotationV1, timeToTilt);
		isTilting = false;
	}
	...
	void LateUpdate()
	{
		...
		if (playerType == PlayerType.rigidBodyPlayer && !spawnManager.HasDied && CANCROUCH && !isJumping)
		{
			CrouchSystem ();
			if (playerCapsuleInitRadius > crouchLocalScaleY)
				decreaseRadiusCrouch = true;
			else
				decreaseRadiusCrouch = false;
			if (isCrouching)
				haltProneScale = true;
			else
				haltProneScale = false;
		}
		if (playerType == PlayerType.rigidBodyPlayer && !spawnManager.HasDied && CANPRONE && !isJumping)
		{		
			ProneSystem ();
			if (playerCapsuleInitRadius > proneLocalScaleY)
				decreaseRadiusProne = true;
			else
				decreaseRadiusProne = false;
			if (isProne)
				haltCrouchScale = true;
			else
				haltCrouchScale = false;
		}
	...
	void SwitchPlayerType ()
	{
		if (playerType == PlayerType.rigidBodyPlayer) {
			...
			if(proneButton != null)
				proneButton.gameObject.SetActive(true);
		...
		} else if (playerType == PlayerType.spectator)
		{
			...
			if(proneButton != null)
				proneButton.gameObject.SetActive(false);
			if(goDownButton != null)
	...
	void Update ()
	{
		...
		if (playerType == PlayerType.rigidBodyPlayer && !spawnManager.HasDied) {
			...
			if (_audioSrc != null) 
			{
				// If not grounded, then set the audiosource volume back to initValue
				if(!isGrounded)
					_audioSrc.volume = audioVolume;
			}
			...
			if (pivot != null)
			{
				// Set the AFPC_PlayerPivot gameobject's position to be on the bottom of the player capsule
			...
			transform.rotation = Quaternion.Euler (0f, fpsCamera.GetComponent<AFPC_Cam> ().CurrentyRotation, zRotation);	
			PlayerStateChecker ();
			currentAcceleration = DesiredAcceleration(new Vector2(inputX, inputY));

			if ((inputX != 0 || inputY != 0) && !isCrouching && !isProne && !isSliding && !isSwimming && !isRunning && !isJumping && rgbd.velocity.sqrMagnitude > 0.1f && !canClimb && isGrounded)
				isWalking = true;
			else
				isWalking = false;
	...
	void PlayerStateChecker()
	{
		...
		if (isCrouching && !isSwimming && !isProne)
			playerState = PlayerStates.Crouch_1;
		if (isProne && !isSwimming && !isCrouching)
			playerState = PlayerStates.Prone_4;
		if (canClimb)
			playerState = PlayerStates.Climb_3;
		if (!canClimb && !isCrouching && !isSwimming && !isProne)
	...
	float DesiredAcceleration(Vector2 input)
	{
		...
		} else if (!isRunning && isGrounded && !isCrouching && !isProne) {
			if (input.y > 0)
		...
		} else if (isGrounded && isCrouching && !isProne) {
			if (input.y > 0)
		...
		} else if (isGrounded && isProne) {
			if (input.y > 0)
				desiredAcceleration = proneAccelerationForward;
			else
				desiredAcceleration = proneAccelerationBackward;

			if (input.x > 0 || input.x < 0)
				desiredAcceleration = proneStrafeAcceleration;
	...
	void InputManagement()
	{
		...
		if (playerType == PlayerType.rigidBodyPlayer) {
			...
			if(Input.GetKeyDown(proneKey) && !proneButtonDown)
				proneButtonDown = true;
		...
		if (playerType == PlayerType.rigidBodyPlayer) {
			...
			{
				proneButton.buttonType = AFPC_VirtualButton.ButtonType.LateTriggerButton;
				if(!proneButtonDown && proneButton.value)
					proneButtonDown = true;
			}
		...
		if (playerType == PlayerType.rigidBodyPlayer && !spawnManager.HasDied) {
			...
			if (canClimb && CANCLIMB) {
				...
				if (useMouseScrollForClimbing && climbKeyHold) {
					...
					cam.doClimbBob = true;
					rgbd.constraints = initialRigidbodyConstraints;	//Reseting the constraints to initial constraints
					if (useClimbingSound && climbingSounds != null && !climbingSoundCoroutineFlag) {
...
//					desiredMove = fpsCamera.transform.forward * inputY + fpsCamera.transform.right * inputX;	//This Causes Errors!!!!!
					
				if (((inputY != 0 || inputX != 0) || (isRunning && runWithoutAxisInput)) && !canClimb && isGrounded && CANMOVE) {
					desiredMove = new Vector3 (fpsCamera.transform.forward.x * inputY + fpsCamera.transform.right.x * inputX, fpsCamera.transform.right.y * inputX, fpsCamera.transform.forward.z * inputY + fpsCamera.transform.right.z * inputX);
					...
					if (!isJumping) 
					{
						if (rgbd.velocity.sqrMagnitude < currentAcceleration * currentAcceleration)
						{
							if (!runWithoutAxisInput)
				...
				if (infiniteRunning) {
					...
					if (runKeyHold && isGrounded && ((inputX != 0 || inputY != 0) || runWithoutAxisInput) && !canClimb && (rgbd.velocity.sqrMagnitude > 0.1f || runWithoutAxisInput)) {
						//Running
						isRunning = true;
				...
				if (jumpPressed && isGrounded && !isCrouching && !isProne && !canClimb && CANJUMP) {
					//Jumping
					if (jumpUpSound != null) {
				...
				if (rgbd.velocity.y < 0 && useFallMultiplier && !isGrounded && !isCrouching && !isProne) {
					rgbd.velocity += Vector3.up * Physics.gravity.y * (fallMultiplier - 1) * Time.deltaTime;		
				} else if (rgbd.velocity.y > 0 && !Input.GetButton ("Jump") && jumpForceDependsOnTheTimeJumpButtonPressed && !canClimb && !isCrouching && !isProne) {
		...
		} else if (playerType == PlayerType.spectator)
		{
			...
			if (!useJoystickInSpectatorMode) 
			{
				if (spectatorKeyHold) {
					float Yfactor = fpsCamera.transform.forward.y; // The acceleration depends on the camera forward direction
					inputY3 = Yfactor * spectatorModeAcceleration;
				} else {
					inputY3 = 0f;
				}
			}
			...
			#endif
			#if UNITY_STANDALONE || UNITY_WEBGL
			rgbd.AddRelativeForce (inputX * spectatorModeAcceleration, inputY3, inputY * spectatorModeAcceleration, ForceMode.Acceleration);
			#endif

			#if UNITY_ANDROID || UNITY_IOS
				if(!useJoystickInSpectatorMode)
				rgbd.AddRelativeForce (0f, inputY3, inputY3, ForceMode.Acceleration);
				else
	...
	void OnDrawGizmos()
	{
		if (visualize)
		{
			...
			//For Prone Above Collision Detection
			Vector3 endOfCapsule = new Vector3 (transform.position.x, 0f, transform.position.z);
			endOfCapsule.y = transform.position.y + (playerCapsuleInitHeight * originalLocalScaleY / 2) + 0.25f + floatOffset;
			Gizmos.DrawLine (transform.position, endOfCapsule);
		}
	}
